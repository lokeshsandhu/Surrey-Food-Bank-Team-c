...
export async function updateAppointment(appt_date: string, start_time: string, updateData: Partial<{ end_time: string; appt_notes: string; username: string }>) {
    ...
    for (const [key, value] of Object.entries(updateData)) {
        fields.push(`${key} = $${idx}`);
        if (key === "appt_notes") {
            values.push(encryptForDb("appointment", "appt_notes", value));
        } else {
            values.push(value);
        }
        idx++;
    ...
    return row ? decryptRowFromDb("appointment", row) : null;
}
    ...
    while (current < end) {
        ...
        const overlap = await hasOverlap(data.appt_date, slotStart, slotEnd);
	        if (!overlap) {
	            ...
	            const values = [data.appt_date, slotStart, slotEnd, encryptForDb("appointment", "appt_notes", data.appt_notes || null)];
	            const { rows } = await pool.query(text, values);
	            slots.push(decryptRowFromDb("appointment", rows[0]));
	        }
        current = next;
    }
	    return slots;
...
export async function createAppointment(data: CreateSlotDTO) {
    ...
    const values = [
        ...
        data.start_time,
        data.end_time,
        encryptForDb("appointment", "appt_notes", data.appt_notes),
    ];
    const { rows } = await pool.query(text, values);
    return decryptRowFromDb("appointment", rows[0]);
}
...
export async function getAvailableAppointments(babyOrPregnant: boolean, username?: string) {
    ...
    if (babyOrPregnant) {
        ...
        if (householdSize >= 4) {
            ...
            const decrypted = decryptRowsFromDb("appointment", rows);
...
export async function bookAppointment(data: BookAppointmentDTO, username: string) {
    ...
    booked.push(decryptRowFromDb("appointment", first[0]));
    if (slotsNeeded === 2) {
        ...
        booked.push(decryptRowFromDb("appointment", second[0]));
    }
